...
func (r Running) Calories() float64 {
	return (CaloriesMeanSpeedMultiplier*r.meanSpeed() +
		CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours() * MinInHours
}
...
func (w Walking) Calories() float64 {
	...
	return (CaloriesWeightMultiplier*w.Weight + (math.Pow(speedMInSec, 2)/(w.Height/CmInM))*
...
// distance возвращает преодоленную дистанцию в километрах
// Формула расчета:
// длина_бассейна * количество_пересечений
...
// Формула расчета:
// преодоленная_дистанция_в_км / продолжительность_тренировки
// Это переопределенный метод Calories() из Training.
